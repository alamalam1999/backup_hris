...
if( isset($_SERVER['REQUEST_METHOD']) AND strtoupper($_SERVER['REQUEST_METHOD'])=='POST' )
{
	...
	if($UPDATE_TYPE == 'DATA_PELAMAR'){
		$foto_allow_ext = array('png','jpg','jpeg');
		...
		$ijazah_allow_ext = array('png','jpg','jpeg','pdf');
		...
		$fcktp_allow_ext = array('png','jpg','jpeg','pdf');
		...
		$fcnpwp_allow_ext = array('png','jpg','jpeg','pdf');
		...
		$fcbpjs_kes_allow_ext = array('png','jpg','jpeg','pdf');
		...
		$fcbpjs_ket_allow_ext = array('png','jpg','jpeg','pdf');
		...
		$fcbpjs_jp_allow_ext = array('png','jpg','jpeg','pdf');
		...
		$REQUIRE = array(
			'NAMA'=>'<b>Nama</b>', 
			'NAMA_PANGGILAN'=>'<b>Nama Panggilan</b>',
			'TP_LAHIR'=>'<b>Tempat Lahir</b>',
			'TGL_LAHIR'=>'<b>Tanggal Lahir</b>',
			'TINGGI'=>'<b>Tinggi Badan</b>',
			'BERAT'=>'<b>Berat Badan</b>',
			'UKURAN_SEPATU'=>'<b>Ukuran Sepatu</b>',
			'HP'=>'<b>Nomor HP</b>',
			'EMAIL'=>'<b>Alamat Email</b>',
			'NO_IDENTITAS'=>'<b>No. e-KTP</b>',
			'ALAMAT_KTP'=>'<b>Alamat Sesuai KTP</b>',
			'KELURAHAN_KTP'=>'<b>Kelurahan Sesuai KTP</b>',
			'KECAMATAN_KTP'=>'<b>Kecamatan Sesuai KTP</b>',
			'PROVINSI_KTP'=>'<b>Provinsi Sesuai KTP</b>',
			'KOTA_KTP'=>'<b>Kota Sesuai KTP</b>',
			'KODE_POS_KTP'=>'<b>Kode Pos Sesuai KTP</b>',
			'RT_KTP'=>'<b>RT Sesuai KTP</b>',
			'RW_KTP'=>'<b>RW Sesuai KTP</b>',
			'ALAMAT'=>'<b>Alamat Tempat tinggal</b>',
			'KELURAHAN'=>'<b>Kelurahan Tempat tinggal</b>',
			'KECAMATAN'=>'<b>Kecamatan Tempat tinggal</b>',
			'PROVINSI'=>'<b>Provinsi Tempat tinggal</b>',
			'KOTA'=>'<b>Kota Tempat tinggal</b>',
			'KODE_POS'=>'<b>Kode Pos Tempat tinggal</b>',
			'RT'=>'<b>RT Tempat tinggal</b>',
			'RW'=>'<b>RW Tempat tinggal</b>'
		);

		$ERROR_REQUIRE = 0;
		foreach($REQUIRE as $REQ => $VAL){
			$IREQ = get_input($REQ);
		...
		if( $ERROR_REQUIRE )
		{
			foreach ($REQUIRE as $REQ => $VAL) {
			 	$IREQ = get_input($REQ);
			 	if($IREQ == "") $ERROR[] = 'kolom '.$VAL.' wajib diisi.';
			}
		}
		...
		else if( get_input('CURRENT_FCKTP')=='' AND ! is_uploaded_file($fcktp_tmp) )
		{
			$ERROR[] = 'Wajib Upload foto e-KTP';
			if( ! in_array($fcktp_ext,$fcktp_allow_ext) )
			{
				$ERROR[] = 'Foto e-KTP harus bertipe png, jpg atau pdf';
			}
		}

		else if( get_input('CURRENT_FOTO')=='' AND ! is_uploaded_file($foto_tmp) )
		{
			$ERROR[] = 'Wajib Upload Foto terbaru';
			if( ! in_array($foto_ext,$foto_allow_ext) ){
				$ERROR[] = 'Foto terbaru harus bertipe jpg atau png';
			}
		}
		
		else if( get_input('CURRENT_IJAZAH')=='' AND ! is_uploaded_file($ijazah_tmp) )
		{
			$ERROR[] = 'Wajib Upload Scan Ijazah';
			if( ! in_array($ijazah_ext,$ijazah_allow_ext) ){
				$ERROR[] = 'Scan Ijazah harus bertipe jpg, png atau pdf';
			}
		}

		else if( get_input('CURRENT_CV')=='' AND ! is_uploaded_file($cv_tmp) )
		{
			$ERROR[] = 'Upload CV wajib di isi';
			...
			if ($OP=='add' || $OP==''){
				$NAMA = db_escape(get_input('NAMA'));
				$NAMA = strtoupper($NAMA);
				$SQL = db_execute(" INSERT INTO calon_karyawan (NAMA,".implode(',',$FIELDS).") VALUES ('$NAMA',".implode(',',$INSERT_VAL).") ");
				$OP = 'edit';
				$ID = $DB->Insert_ID();
	...
	if($UPDATE_TYPE == 'DATA_POSISI'){
		$REQUIRE = array(
			'POSISI_ID'=>'<b>Jabatan</b>', 
			'LOWONGAN_ID'=>'<b>Lowongan</b>',
			'APPLICANT_NO'=>'<b>Applicant No</b>'
		);
		...
		if( $ERROR_REQUIRE )
		{
			...
			$APPLICANT_NO = db_escape(get_input('APPLICANT_NO'));
	...
	if($UPDATE_TYPE == 'DATA_PENDIDIKAN'){
		...
		$TINGKAT = db_escape(get_input('TINGKAT'));
		if(is_array($TINGKAT) AND count($TINGKAT)){
			foreach($TINGKAT as $key=>$val){
				if($val != ''){
					$JURUSAN 			= db_escape(get_input('JURUSAN'));
					$INSTITUSI 			= db_escape(get_input('INSTITUSI'));
					$LOKASI 			= db_escape(get_input('LOKASI'));
					$TAHUN_MULAI 		= db_escape(get_input('TAHUN_MULAI'));
					$TAHUN_SELESAI 		= db_escape(get_input('TAHUN_SELESAI'));
					$GPA 				= db_escape(get_input('GPA'));
		...
		$NAMA_KURSUS = db_escape(get_input('NAMA_KURSUS'));
		if(is_array($NAMA_KURSUS) AND count($NAMA_KURSUS)){
			foreach($NAMA_KURSUS as $key=>$val){
				if($val != ''){
					$TEMPAT 			= db_escape(get_input('TEMPAT'));
					$PERIODE_MULAI 		= db_escape(get_input('PERIODE_MULAI'));
					$PERIODE_SELESAI 	= db_escape(get_input('PERIODE_SELESAI'));
		...
		$BAHASA = db_escape(get_input('BAHASA'));
		if(is_array($BAHASA) AND count($BAHASA)){
			foreach($BAHASA as $key=>$val){
				if($val != ''){
					$LISAN 				= db_escape(get_input('LISAN'));
					$TULISAN 			= db_escape(get_input('TULISAN'));
					$CALON_KARYAWAN_ID	= $ID;
					$BAHASA 			= $val;
	...
	if($UPDATE_TYPE == 'DATA_ORGANISASI'){
		...
		$NAMA_ORGANISASI = db_escape(get_input('NAMA_ORGANISASI'));
		if(is_array($NAMA_ORGANISASI) AND count($NAMA_ORGANISASI)){
			foreach($NAMA_ORGANISASI as $key=>$val){
				if($val != ''){
					$JABATAN_ORGANISASI = db_escape(get_input('JABATAN_ORGANISASI'));
					$LOKASI_ORGANISASI 	= db_escape(get_input('LOKASI_ORGANISASI'));
					$PERIODE_ORGANISASI = db_escape(get_input('PERIODE_ORGANISASI'));
					$CALON_KARYAWAN_ID	= $ID;
					$NAMA_ORGANISASI 	= $val;
	...
	if($UPDATE_TYPE == 'DATA_KELUARGA'){
		...
		$ANGGOTA_KELUARGA_INTI = db_escape(get_input('ANGGOTA_KELUARGA_INTI'));
		if(is_array($ANGGOTA_KELUARGA_INTI) AND count($ANGGOTA_KELUARGA_INTI)){
			foreach($ANGGOTA_KELUARGA_INTI as $key=>$val){
				if($val != ''){
					$NAMA_KELUARGA_INTI 		= db_escape(get_input('NAMA_KELUARGA_INTI'));
					$GENDER_INTI 				= db_escape(get_input('GENDER_INTI'));
					$TP_LAHIR_KELUARGA_INTI 	= db_escape(get_input('TP_LAHIR_KELUARGA_INTI'));
					$TGL_LAHIR_KELUARGA_INTI 	= db_escape(get_input('TGL_LAHIR_KELUARGA_INTI'));
					$PENDIDIKAN_KELUARGA_INTI 	= db_escape(get_input('PENDIDIKAN_KELUARGA_INTI'));
					$PEKERJAAN_KELUARGA_INTI 	= db_escape(get_input('PEKERJAAN_KELUARGA_INTI'));
					$CALON_KARYAWAN_ID			= $ID;
		...
		$ANGGOTA_KELUARGA_BESAR = db_escape(get_input('ANGGOTA_KELUARGA_BESAR'));
		if(is_array($ANGGOTA_KELUARGA_BESAR) AND count($ANGGOTA_KELUARGA_BESAR)){
			foreach($ANGGOTA_KELUARGA_BESAR as $key=>$val){
				if($val != ''){
					$NAMA_KELUARGA_BESAR 		= db_escape(get_input('NAMA_KELUARGA_BESAR'));
					$GENDER_BESAR 				= db_escape(get_input('GENDER_BESAR'));
					$TP_LAHIR_KELUARGA_BESAR 	= db_escape(get_input('TP_LAHIR_KELUARGA_BESAR'));
					$TGL_LAHIR_KELUARGA_BESAR 	= db_escape(get_input('TGL_LAHIR_KELUARGA_BESAR'));
					$PENDIDIKAN_KELUARGA_BESAR 	= db_escape(get_input('PENDIDIKAN_KELUARGA_BESAR'));
					$PEKERJAAN_KELUARGA_BESAR 	= db_escape(get_input('PEKERJAAN_KELUARGA_BESAR'));
					$CALON_KARYAWAN_ID			= $ID;
	...
	if($UPDATE_TYPE == 'DATA_PENGALAMAN'){
		...
		$NAMA_PERUSAHAAN = db_escape(get_input('NAMA_PERUSAHAAN'));
		if(is_array($NAMA_PERUSAHAAN) AND count($NAMA_PERUSAHAAN)){
			foreach($NAMA_PERUSAHAAN as $key=>$val){
				if($val != ''){
					$BIDANG_USAHA			= db_escape(get_input('BIDANG_USAHA'));
					$ALAMAT_PERUSAHAAN		= db_escape(get_input('ALAMAT_PERUSAHAAN'));
					$ATASAN 				= db_escape(get_input('ALAMAT_PERUSAHAAN'));
					$NO_TELP_PERUSAHAAN		= db_escape(get_input('NO_TELP_PERUSAHAAN'));
					$PERIODE_BEKERJA		= db_escape(get_input('PERIODE_BEKERJA'));
					$JABATAN_AWAL			= db_escape(get_input('JABATAN_AWAL'));
					$JABATAN_AKHIR			= db_escape(get_input('JABATAN_AKHIR'));
					$GAPOK_SEBELUMNYA		= db_escape(input_currency(get_input('GAPOK_SEBELUMNYA')));
					$TUNJANGAN_LAINNYA		= db_escape(get_input('TUNJANGAN_LAINNYA'));
					$ALASAN_RESIGN			= db_escape(get_input('ALASAN_RESIGN'));
					$DESKRIPSI_PEKERJAAN	= db_escape(get_input('DESKRIPSI_PEKERJAAN'));
					$CALON_KARYAWAN_ID		= $ID;
					$NAMA_PERUSAHAAN 		= $val;
	...
	if($UPDATE_TYPE == 'DATA_LAIN'){
		...
		$NAMA_PENANGGUNG = db_escape(get_input('NAMA_PENANGGUNG'));
		if(is_array($NAMA_PENANGGUNG) AND count($NAMA_PENANGGUNG)){
			foreach($NAMA_PENANGGUNG as $key=>$val){
				if($val != ''){
					$ALAMAT_PENANGGUNG		= db_escape(get_input('ALAMAT_PENANGGUNG'));
					$TELP_PENANGGUNG		= db_escape(get_input('TELP_PENANGGUNG'));
					$HUBUNGAN_PENANGGUNG 	= db_escape(get_input('HUBUNGAN_PENANGGUNG'));
					$CALON_KARYAWAN_ID		= $ID;
					$NAMA_PENANGGUNG 		= $val;
...
<section class="container-fluid" style="margin-top:10px;">
	<h1 class="border-title">Isi Data dengan Lengkap</h1>
	<?php include 'msg-public.php' ?>
	...
	<form id="form" class="form-horizontal" action="lamaran-public.php?op=<?php echo $OP ?>&id=<?php echo $ID ?>&tab=<?php echo $TAB ?>" method="POST" enctype="multipart/form-data">
		...
		<div class="tab-content" style="margin-top:20px;">
				...
				<div class="row">
					<div class="col-md-6">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<?php echo dropdown('PUNYA_KENDARAAN',array('PUNYA'=>'PUNYA','TIDAK'=>'TIDAK'),set_value('PUNYA_KENDARAAN', $EDIT->PUNYA_KENDARAAN),' class="form-control" id="PUNYA_KENDARAAN"') ?>
							</div>
						</div>
						<div class="form-group punya-kendaraan">
							<label class="col-sm-3 control-label">Jenis Kendaraan<span style="color:red; padding-left: 5px;">*</span></label>
							<div class="col-sm-9">
								<?php echo dropdown('JENIS_KENDARAAN',array('MOTOR'=>'MOTOR','MOBIL'=>'MOBIL'),set_value('JENIS_KENDARAAN', $EDIT->JENIS_KENDARAAN),' class="form-control"') ?>
							</div>
						</div>
						<div class="form-group punya-kendaraan">
							<label class="col-sm-3 control-label">Kendaraan Milik<span style="color:red; padding-left: 5px;">*</span></label>
							<div class="col-sm-9">
								<?php echo dropdown('MILIK_KENDARAAN',array('SENDIRI'=>'SENDIRI','ORANG TUA'=>'ORANG TUA','KANTOR'=>'KANTOR'),set_value('MILIK_KENDARAAN', $EDIT->MILIK_KENDARAAN),' class="form-control"') ?>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label"> </label>
							<div class="col-sm-9">
							<button name="UPDATE_TYPE" type="submit" value="DATA_PELAMAR" class="btn btn-primary col-sm-12" onclick="$('#form').submit()">
								<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
							</button>
							</div>
						</div>
					</div>
					...
					<div class="col-md-12">
						...
						<div class="form-group">
							...
							<div class="col-sm-8 text-right">
								<button name="UPDATE_TYPE" type="submit" value="DATA_POSISI" class="btn btn-primary col-sm-6" onclick="$('#form').submit()">
									<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
								</button>
							</div>
						</div>
			...
			<div role="tabpanel" class="tab-pane" id="tab3">
				...
				<div class="form-group container">
					<div class="form-group">
						<div class="col-sm-8 text-right">
							<button name="UPDATE_TYPE" type="submit" value="DATA_PENDIDIKAN" class="btn btn-primary col-sm-6" onclick="$('#form').submit()">
								<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
							</button>
						</div>
					</div>
			...
			<div role="tabpanel" class="tab-pane" id="tab4">
				<div class="row">
					...
					<div class="form-group container">
						<div class="col-sm-8">
							<button name="UPDATE_TYPE" type="submit" value="DATA_ORGANISASI" class="btn btn-primary col-sm-6" onclick="$('#form').submit()">
								<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
							</button>
						</div>
					</div>
			...
			<div role="tabpanel" class="tab-pane" id="tab5">
				<div class="row">
					...
					<div class="form-group container">
						<div class="col-sm-8">
							<button name="UPDATE_TYPE" type="submit" value="DATA_KELUARGA" class="btn btn-primary col-sm-6" onclick="$('#form').submit()">
								<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
							</button>
						</div>
					</div>
			...
			<div role="tabpanel" class="tab-pane" id="tab6">
				...
				<div class="col-md-12 pengalaman">
					<table class="table table-bordered">
						<tr style="background-color: #E9ECEF; color: #495057;">
							<th>Pengalaman kerja, dimulai dari pekerjaan Anda yang terakhir (saat ini)</th>
				...
				<div class="form-group container">
					<div class="col-sm-8">
						<button name="UPDATE_TYPE" type="submit" value="DATA_PENGALAMAN" class="btn btn-primary col-sm-6" onclick="$('#form').submit()">
							<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
						</button>
					</div>
				</div>
			...
			<div role="tabpanel" class="tab-pane" id="tab7">
				<div class="row">
					<div class="col-md-12">
						...
						<div class="form-group container">
							<div class="col-sm-8">
								<button name="UPDATE_TYPE" type="submit" value="DATA_MINAT" class="btn btn-primary col-sm-6" onclick="$('#form').submit()">
									<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
								</button>
							</div>
						</div>
			...
			<div role="tabpanel" class="tab-pane" id="tab8">
				<div class="row">
					...
					<div class="col-sm-8">
						<button name="UPDATE_TYPE" type="submit" value="DATA_LAIN" class="btn btn-primary col-sm-6" onclick="$('#form').submit()">
							<span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save To Next
						</button>
					</div>
				</div>
...
$(document).ready(function(){
	...
	PK = $('#PUNYA_KENDARAAN').val();

	if(PK=='PUNYA'){ 
		$('.punya-kendaraan').show();
	}
	if(PK=='TIDAK'){ 
		$('.punya-kendaraan').hide();
	}

	$('#PUNYA_KENDARAAN').change(function(){
		PK = $(this).val();
		if(PK=='PUNYA'){ 
			$('.punya-kendaraan').show();
		}
