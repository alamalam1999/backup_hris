...
if($OP=='edit'){
	is_login('angsuran.edit');
	$EDIT = db_first(" SELECT * FROM angsuran WHERE ANGSURAN_ID='$ID' ");
	if( isset($EDIT->STATUS) AND $EDIT->STATUS == 'APPROVED' )
	{
		header('location: angsuran.php?m=approved');
		exit;
	}
	if( isset($EDIT->STATUS) AND $EDIT->STATUS == 'VOID' )
	{
		header('location: angsuran.php?m=void');
		exit;
	}
}

if($OP=='approve'){
	is_login('angsuran.change_status');
	$IDS = get_input('ids');
	if( is_array($IDS) )
	{
		$CU = current_user();
		$TIME = date('Y-m-d H:i:s');
		db_execute(" UPDATE angsuran SET STATUS='APPROVED',UPDATED_BY='$CU->NAMA',UPDATED_ON='$TIME',APPROVED_BY='$CU->NAMA',APPROVED_ON='$TIME' WHERE ANGSURAN_ID IN (".implode(',',$IDS).")");
	}
...
if($OP=='delete'){
	is_login('angsuran.delete');
	$REASON = db_escape(get_input('reason'));
	db_execute(" UPDATE angsuran SET STATUS='VOID',KETERANGAN='Alasan Void : $REASON' WHERE ANGSURAN_ID='$ID' ");
	header('location: angsuran.php');
	exit;
}
...
if( isset($_SERVER['REQUEST_METHOD']) AND strtoupper($_SERVER['REQUEST_METHOD'])=='POST' )
{
	...
	if( $ERROR_REQUIRE ){
		...
		$FIELDS = array(
			'PERIODE_ID','KARYAWAN_ID','KETERANGAN','TANGGAL','JENIS'
		);
		...
		if(has_access('angsuran.change_status'))
		{
		...
		if($OP=='' OR $OP=='add')
		{
			is_login('angsuran.add');
			db_execute(" INSERT INTO angsuran (".implode(',',$FIELDS).") VALUES (".implode(',',$INSERT_VAL).") ");
			$OP = 'edit';
			$ID = $DB->Insert_Id();
		...
		{
			db_execute(" UPDATE angsuran SET ".implode(',',$UPDATE_VAL)." WHERE ANGSURAN_ID='$ID' ");
...
<section class="container-fluid" style="margin-top:70px;">
	<h1 class="border-title">
		...
		<?php if($OP=='edit'){ echo '<a href="angsuran-action.php?op=add" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;Add New</a>'; } ?>
	</h1>
	<?php include 'msg.php' ?>
	<div style="border-top:1px dashed #cccccc; margin-top:15px;">&nbsp;</div>
	<form id="form" class="form-horizontal" action="angsuran-action.php?op=<?php echo $OP ?>&id=<?php echo $ID ?>" method="POST">
	<div class="row">
		<div class="col-md-5">
		...
		<div class="col-md-5">
			<div class="form-group">
				<label for="" class="col-sm-3 control-label">Jenis Angsuran</label>
				<div class="col-sm-9">
					<?php echo dropdown('JENIS',array('PERUSAHAAN'=>'PERUSAHAAN','PIHAK KETIGA'=>'PIHAK KETIGA'),set_value('STATUS',$EDIT->STATUS),' class="form-control" ') ?>
				</div>
			</div>
			<div class="form-group">
			...
			<?php if(has_access('angsuran.change_status')){ ?>
			<div class="form-group">
