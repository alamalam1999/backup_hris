	...
	<div class="row" style="margin:10px 0;">
		<div class="col-sm-2">
			<div class="dropdown">
				...
				<li><a href="adjusment-action.php?op=add" style=""><i class="fa fa-plus"></i>&nbsp;&nbsp;Add</a></li>
				...
				<?php if(has_access('adjusment.delete')){ ?>
		...
		<div class="col-sm-2">
			<?php echo dropdown('PERIODE_ID',dropdown_option('periode','PERIODE_ID','PERIODE','ORDER BY PERIODE_ID DESC'),get_search('ADJUSMENT','PERIODE_ID'),' id="PERIODE_ID" class="form-control input-sm" ') ?>
		</div>
		<div class="col-sm-2">
			<?php echo dropdown('PROJECT_ID',project_option_filter(0),get_search('ADJUSMENT','PROJECT_ID'),' id="PROJECT_ID" class="form-control input-sm" ') ?>
		</div>
		<div class="col-sm-2">
			<input type="text" id="NAMA" value="<?php echo get_search('ADJUSMENT','NAMA') ?>" class="form-control input-sm" placeholder="Nama....">
		</div>
		<div class="col-sm-4">
			<h1 style="margin:0;text-align:right;">Adjusment</h1>
		</div>
	</div>
...
$(document).ready(function(){
	...
	$('#btn-edit').click(function(){
		...
		else window.location = 'adjusment-action.php?op=edit&id='+sel.ADJUSMENT_ID;
		return false;
	});
	$('#btn-delete').click(function(){
		...
		else if(confirm('Are you sure?'))
		{
			...
			if (REASON != null) {
				window.location = 'adjusment-action.php?op=delete&id='+sel.ADJUSMENT_ID+'&reason='+REASON;
			}
		}
		return false;
	});
	...
	$('#btn-approved').click(function(){
		...
		if( sel == undefined ) alert('No item selected');
			...
			$.each(rows, function(index, value){
				QS += '&ids[]=' + value.ADJUSMENT_ID;
			});
			window.location = 'adjusment-action.php?op=approve' + QS;
		}
		return false;
	});
