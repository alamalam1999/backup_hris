...
if (isset($_GET['generate'])) {
	...
	if (count($karyawan) > 0) {
		foreach ($karyawan as $k) {
			...
			$SCAN = parse_scan_new_app($k->KARYAWAN_ID, $TGL_MULAI, $TGL_SELESAI, $SHIFT);
			$SCAN_PAYROLL = $SCAN;
			...
			if ($CUTOFF == '1') {
				...
				$SCAN_PAYROLL = parse_scan_new_app($k->KARYAWAN_ID, $TGL_MULAI2, $TGL_SELESAI2, $SHIFT2);
			}
			...
			*/
			...
			$TUNJ_MAKAN = 0;
			if ($k->R_TUNJ_MAKAN == '1') {
				$TOTAL_ATT = isset($SCAN['total_working_day']) ? $SCAN['total_working_day'] : 0;
				...
				if ($k->R_POT_EARLY_TUNJ_MAKAN == '1') {
					...
					$TUNJ_MAKAN = $TUNJ_MAKAN - ($k->TUNJ_MAKAN * $TOTAL_EARLY);
					if ($TUNJ_MAKAN < 0) $TUNJ_MAKAN = 0;
				}
			...
			if ($k->R_TUNJ_TRANSPORT == '1') {
				...
				if ($k->R_POT_LATE_TUNJ_TRANSPORT == '1') {
					$TOTAL_LATE = isset($SCAN['total_late']) ? $SCAN['total_late'] : 0;
					$TUNJ_TRANSPORT = $TUNJ_TRANSPORT - ($k->TUNJ_TRANSPORT * $TOTAL_LATE);
					if ($TUNJ_TRANSPORT < 0) $TUNJ_TRANSPORT = 0;
				}

				if ($k->R_POT_EARLY_TUNJ_TRANSPORT == '1') {
					$TOTAL_EARLY = isset($SCAN['total_early']) ? $SCAN['total_early'] : 0;
					$TUNJ_TRANSPORT = $TUNJ_TRANSPORT - ($k->TUNJ_TRANSPORT * $TOTAL_EARLY);
					if ($TUNJ_TRANSPORT < 0) $TUNJ_TRANSPORT = 0;
				}
			...
			$LHK = $LHL = $IHB = 0;
			if ($k->R_LEMBUR == '1') {
				$LEMBUR_LHK = db_first(" SELECT SUM(UANG_LEMBUR) AS UANG_LEMBUR FROM lembur WHERE KARYAWAN_ID='$k->KARYAWAN_ID' AND PERIODE_ID='$PERIODE_ID' AND STATUS='APPROVED' AND JENIS='LHK' AND STAFF=1 ")->UANG_LEMBUR;

				$LEMBUR_LHL = db_first(" SELECT SUM(UANG_LEMBUR) AS UANG_LEMBUR FROM lembur WHERE KARYAWAN_ID='$k->KARYAWAN_ID' AND PERIODE_ID='$PERIODE_ID' AND STATUS='APPROVED' AND JENIS='LHL' AND STAFF=1 ")->UANG_LEMBUR;
			...
			$TOTAL_TUNJANGAN_TANPA_TMP = $TUNJ_OTTW + $TUNJ_LAINNYA_1 + $TUNJ_LAINNYA_2 + $TUNJ_LAINNYA_3 + $TUNJ_LAINNYA_4 + $TUNJ_LAINNYA_5  + $TUNJ_SHIFT + $TUNJ_BACKUP + $TUNJ_KEHADIRAN + $LHK + $LHL + $IHB + $MEDICAL;
			...
			$TOTAL_GAJI_KOTOR = $GAJI_POKOK_NET + $TOTAL_TUNJANGAN_TANPA_TMP - $TOTAL_POTONGAN;
