...
$MODULE = 'EQUIPMENT-STOCK';
is_login('equipment-stock.view');
...
$where = '';
$wh = array();
if($NAMA = get_search($NAMA,'NAMA') AND !empty($NAMA)) $wh[] = " UCASE(NAMA) LIKE UCASE('%$NAMA%') ";
if(count($wh)>0) $where = " WHERE " . implode(' AND ',$wh);
	
$DATA = db_fetch_limit("
	SELECT A.NAMA, C.COMPANY, E.EQUIPMENT_STOCK_ID, E.EQUIPMENT_ID, E.SALDO, E.TGL FROM
	(
	    SELECT max(EQUIPMENT_STOCK_ID) as EQUIPMENT_STOCK_ID, EQUIPMENT_ID  FROM equipment_stock GROUP BY EQUIPMENT_ID
	) temp LEFT JOIN equipment_stock E on E.EQUIPMENT_ID = temp.EQUIPMENT_ID AND E.EQUIPMENT_STOCK_ID = temp.EQUIPMENT_STOCK_ID
	LEFT JOIN equipment A ON E.EQUIPMENT_ID= A.EQUIPMENT_ID
...
<section class="container-fluid" style="margin-top:70px;">
	<h1>
		<span style="color:#aaaaaa;">Equipment</span> - Stock
		<?php /*<a href="master-equipment-action.php?op=add" class="btn btn-primary" style="margin-left:10px;"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;Add New</a>*/ ?>
	</h1>

	<form id="form" action="master-equipment.php" method="GET">
	...
	<tr>
		...
		<td style="text-align:center;">
			<a href="equipment-stock-action.php?op=add&id=<?php echo $row->EQUIPMENT_ID ?>" title="Update"><span class="label label-primary">UPDATE</span></a>
			<?php /*
			<a href="master-equipment-action.php?op=edit&id=<?php echo $row->EQUIPMENT_ID ?>" onclick="return confirm('Yakin data akan di hapus?')" title="Delete">
