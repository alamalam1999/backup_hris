...
if($OP=='edit'){
	is_login('kelebihan-jamajar.edit');
	$EDIT = db_first(" SELECT * FROM kelebihan_jamajar WHERE KEL_JAM_AJAR_ID='$ID' ");
	if( isset($EDIT->STATUS) AND $EDIT->STATUS == 'APPROVED' )
	{
		header('location: kelebihan-jamajar.php?m=approve');
		exit;
	}
	if( isset($EDIT->STATUS) AND $EDIT->STATUS == 'VOID' )
	{
		header('location: kelebihan-jamajar.php?m=void');
		exit;
	}
}

if($OP=='approve'){
	is_login('kelebihan-jamajar.approve');
	$IDS = get_input('ids');
	if( is_array($IDS) )
	{
		$CU = current_user();
		$TIME = date('Y-m-d H:i:s');
		db_execute(" UPDATE kelebihan_jamajar SET STATUS='APPROVED',UPDATED_BY='$CU->NAMA',UPDATED_ON='$TIME',APPROVED_BY='$CU->NAMA',APPROVED_ON='$TIME' WHERE KEL_JAM_AJAR_ID IN (".implode(',',$IDS).")");
	}
	header('location: kelebihan-jamajar.php');
	exit;
}

if($OP=='delete'){
	is_login('kelebihan-jamajar.delete');
	$REASON = db_escape(get_input('reason'));
	db_execute(" UPDATE kelebihan_jamajar SET STATUS='VOID',KETERANGAN='Alasan Void : $REASON' WHERE KEL_JAM_AJAR_ID='$ID' ");
	header('location: kelebihan-jamajar.php');
	exit;
}
...
if( isset($_SERVER['REQUEST_METHOD']) AND strtoupper($_SERVER['REQUEST_METHOD'])=='POST' )
{
	...
	if( $ERROR_REQUIRE ){
		...
		if(has_access('kelebihan-jamajar.approve'))
		{
		...
		$rs = db_first(" SELECT J.PROJECT_ID, J.KELEBIHAN_JAM_AJAR FROM karyawan K LEFT JOIN jabatan J ON J.JABATAN_ID=K.JABATAN_ID WHERE KARYAWAN_ID='$KARYAWAN_ID'  ");
		$R_KELEBIHAN = $rs->KELEBIHAN_JAM_AJAR;
		$NILAI_KELEBIHAN = $R_KELEBIHAN * get_input('TOTAL');
		$FIELDS['NILAI'] = 'NILAI';
		$INSERT_VAL['NILAI'] = "'".$NILAI_KELEBIHAN."'";
		$UPDATE_VAL['NILAI'] = "NILAI='".$NILAI_KELEBIHAN."'";

		$FIELDS['RATE'] = 'RATE';
		$INSERT_VAL['RATE'] = "'".$R_KELEBIHAN."'";
		$UPDATE_VAL['RATE'] = "RATE='".$R_KELEBIHAN."'";
		//echo $NILAI_KELEBIHAN; die();
		...
		if($OP=='' OR $OP=='add')
		{
			is_login('kelebihan-jamajar.add');
			//echo "string"; die();
			db_execute(" INSERT INTO kelebihan_jamajar (".implode(',',$FIELDS).") VALUES (".implode(',',$INSERT_VAL).") ");
			$OP = 'edit';
			$ID = $DB->Insert_Id();
		...
		{
			db_execute(" UPDATE kelebihan_jamajar SET ".implode(',',$UPDATE_VAL)." WHERE KEL_JAM_AJAR_ID='$ID' ");
...
<section class="container-fluid" style="margin-top:70px;">
	<h1 class="border-title">
		<?php echo ucfirst($OP) ?> Kelebihan Jam Ajar
		...
		<?php if($OP=='edit'){ echo '<a href="kelebihan-jamajar-action.php?op=add" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;Add New</a>'; } ?>
	</h1>
	<?php include 'msg.php' ?>
	<div style="border-top:1px dashed #cccccc; margin-top:15px;">&nbsp;</div>
	<form id="form" class="form-horizontal" action="kelebihan-jamajar-action.php?op=<?php echo $OP ?>&id=<?php echo $ID ?>" method="POST">
	<div class="row">
		<div class="col-md-5">
		...
		<div class="col-md-5">
			...
			<?php if(has_access('kelebihan-jamajar.approve')){ ?>
			<div class="form-group">
...
$(document).ready(function(){
	$('input[name=KELEBIHAN_JAMAJAR]').focus();
