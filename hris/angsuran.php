	...
	<div class="row" style="margin:10px 0;">
		<div class="col-sm-2">
			<div class="dropdown">
				...
				<li><a href="angsuran-action.php?op=add" style=""><i class="fa fa-plus"></i>&nbsp;&nbsp;Add</a></li>
				<li><a href="javascript:void(0)" id="btn-edit" style=""><i class="fa fa-pencil"></i>&nbsp;&nbsp;Edit</a></li>
				<li><a href="angsuran-import.php" id="btn-import" style=""><i class="fa fa-file-excel-o"></i>&nbsp;&nbsp;Import</a></li>
				...
				<?php if(has_access('angsuran.delete')){ ?>
		...
		<div class="col-sm-2">
			<?php echo dropdown('PERIODE_ID',dropdown_option('periode','PERIODE_ID','PERIODE','ORDER BY PERIODE_ID DESC'),get_search('ANGSURAN','PERIODE_ID'),' id="PERIODE_ID" class="form-control input-sm" ') ?>
		</div>
		<div class="col-sm-2">
			<?php echo dropdown('PROJECT_ID',project_option_filter(0),get_search('ANGSURAN','PROJECT_ID'),' id="PROJECT_ID" class="form-control input-sm" ') ?>
		</div>
		<div class="col-sm-2">
			<input type="text" id="NAMA" value="<?php echo get_search('ANGSURAN','NAMA') ?>" class="form-control input-sm" placeholder="Nama....">
		</div>
		<div class="col-sm-4">
			<h1 style="margin:0;text-align:right;">Angsuran</h1>
		</div>
	</div>
...
$(document).ready(function(){
	...
	$('#btn-edit').click(function(){
		...
		else window.location = 'angsuran-action.php?op=edit&id='+sel.ANGSURAN_ID;
		return false;
	});
	$('#btn-delete').click(function(){
		...
		{
			var REASON = prompt("Kenapa angsuran ini di VOID ?", "Alasan");
			if (REASON != null) {
				window.location = 'angsuran-action.php?op=delete&id='+sel.ANGSURAN_ID+'&reason='+REASON;
			}
		}
		return false;
	});

	/* quick approved angsuran */
	$('#btn-approved').click(function(){
		...
		if( sel == undefined ) alert('No item selected');
			...
			$.each(rows, function(index, value){
				QS += '&ids[]=' + value.ANGSURAN_ID;
			});
			window.location = 'angsuran-action.php?op=approve' + QS;
		}
		return false;
	});
	/* end quick approved angsuran */ 
