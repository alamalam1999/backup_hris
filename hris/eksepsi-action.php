...
if ($OP == 'approve') {
	...
	$rs = db_fetch(" SELECT EKSEPSI_ID  
		FROM eksepsi 
		WHERE EKSEPSI_ID IN (" . implode(',', $IDS) . ") AND STATUS = 'PENDING'
	");

	$EKSEPSI_ID = array();
	if (count($rs) > 0) {
		foreach ($rs as $row) {
			$EKSEPSI_ID[] = $row->EKSEPSI_ID;
		}
	}
	...
	*/
	...
	if (is_array($EKSEPSI_ID)) {
		$CU = current_user();
		$TIME = date('Y-m-d H:i:s');
		db_execute(" UPDATE eksepsi SET STATUS='APPROVED', UPDATED_BY='$CU->NAMA', UPDATED_ON='$TIME', APPROVED_BY='$CU->NAMA', APPROVED_ON='$TIME' WHERE EKSEPSI_ID IN (" . implode(',', $EKSEPSI_ID) . ")");
	}
	header('location: eksepsi.php');
	exit;
}
...
if (isset($_SERVER['REQUEST_METHOD']) and strtoupper($_SERVER['REQUEST_METHOD']) == 'POST') {
	...
	if ($ERROR_REQUIRE) {
		...
		$FIELDS = array(
			'PERIODE_ID', 'KARYAWAN_ID', 'JENIS', 'KETERANGAN', 'TGL_MULAI', 'TGL_SELESAI', 'JENIS_DINAS', 'TRANSPORT_DINAS'
		);
		...
		/*if(isset($EDIT->EKSEPSI_ID))
	{
		...
		<div class="row">
			...
			<?php }else{*/ ?>
				...
				<div class="form-group" id="JENIS_DINAS_FORM" style="display:none;">
					<label for="" class="col-sm-4 control-label">Jenis Dinas</label>
					<div class="col-sm-8">
						<?php echo dropdown('JENIS_DINAS', array('PENUGASAN' => 'Penugasan', 'Perdin' => 'Perdin'), set_value('JENIS_DINAS', $EDIT->JENIS_DINAS), ' class="form-control" id="JENIS_DINAS" ') ?>
					</div>
				</div>
			...
			");
				...
				<div class="form-group" style="display:none;" id="TRANSPORT_DINAS">
					<label for="" class="col-sm-3 control-label">Transportasi Dinas</label>
					<div class="col-sm-9">
						<input type="text" name="TRANSPORT_DINAS" value="<?php echo set_value('TRANSPORT_DINAS', $EDIT->TRANSPORT_DINAS) ?>" class="form-control">
					</div>
				</div>
...
<script>
	...
	function jenis() {
		...
		if (JENIS == 'DINAS') {
			$('#JENIS_DINAS_FORM').show();
		} else {
			$('#JENIS_DINAS_FORM').hide();
		}

		if (JENIS == 'DINAS' && JENIS_DINAS == 'Perdin') {
			$('#TRANSPORT_DINAS').show();
		} else {
			$('#TRANSPORT_DINAS').hide();
		}
	}
