...
{
	$REQUIRE = array('DATE','TIME','KARYAWAN_ID');
	$ERROR_REQUIRE = 0;
	...
	if( $ERROR_REQUIRE ){
		...
		$TIME = get_input('TIME');
		$DATETIME = $DATE.' '.$TIME;
		db_execute(" INSERT IGNORE log_mesin (PIN,DATE) VALUES ('$KARYAWAN_ID','$DATETIME') ");
...
<section class="container-fluid" style="margin-top:70px;">
	<h1 class="border-title">
		<?php echo ucfirst($OP) ?> Scan Tracking
		<button class="btn btn-primary" onclick="$('#form').submit()" style="margin-left:10px;"><i class="fa fa-save"></i>&nbsp;&nbsp;Save</button>
	</h1>
	<?php include 'msg.php' ?>
	
	<div style="border-top:1px dashed #cccccc; margin-top:15px;">&nbsp;</div>
	
	<form id="form" class="form-horizontal" action="scan-modif.php" method="POST">
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<input type="text" name="TIME" value="<?php echo set_value('TIME') ?>" class="form-control time" autocomplete="off">
			</div>
		</div>
	</form>
