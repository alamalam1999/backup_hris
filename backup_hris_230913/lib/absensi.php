...
{
  $rs = db_fetch(" SELECT *, DATE(A.TANGGAL_ABSEN) AS TANGGAL 
    FROM log_online A
      WHERE
        STATUS='APPROVED' AND
        PIN='$PIN' AND
        (DATE(TANGGAL_ABSEN) >= '$date1' AND DATE(TANGGAL_ABSEN) <= '$date2')
  ");

  $ONLINE = array();
  if (count($rs) > 0) {
    foreach ($rs as $row) {
      $ONLINE[$row->TANGGAL][$row->JENIS_ABSEN] = $row;
    }
  }
...
function hitung_lembur_LHK($JAM, $GAJI_POKOK, $ADJ = 0)
{
  ...
  if ($RUNNING > 0) {
    $PENGALI1 = 1;
    $POINT1   = $PENGALI1 * 1.5; //1.5
    $RUNNING  = $RUNNING - $PENGALI1; //1.5
  }
  if ($RUNNING > 0) {
    $PENGALI2 = $RUNNING; //1.5
    $POINT2   = $PENGALI2 * 2; //3
  }
  // echo 'ADJ  '.$ADJ.'<br>'; 
  ...
  $LHK         = $ADJ * round((1 / 173) * $GAJI_POKOK, 0);
  $ret         = array(
    'ADJ' => $ADJ,
    'PENGALI1' => $PENGALI1,
    'PENGALI2' => $PENGALI2,
