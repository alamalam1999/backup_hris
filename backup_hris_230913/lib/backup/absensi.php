...
function parse_scan_new_app($PIN, $date1, $date2, $SHIFT)
{
  $PIN        = db_escape($PIN);
  $date1      = db_escape($date1);
  ...
  while ($time1 <= $time2) {
    ...
    if (isset($ONLINE[$loop_date][1])) {
      $absent                       = '';
      $no_att_minute                = 0;
      $result[$loop_date]['online'] = 1;
      $note                         = 'Online Absent Via Hadir';

      $scan_in = strtotime($ONLINE[$loop_date][1]->TGL_ABSEN);
    }

    if (isset($ONLINE[$loop_date][2])) {
      $absent                       = '';
      $no_att_minute                = 0;
      $result[$loop_date]['online'] = 1;
      $note                         = 'Online Absent Via Hadir';

      $scan_out = strtotime($ONLINE[$loop_date][2]->TGL_ABSEN);
    }

    if (isset($ONLINE[$loop_date][1]) && isset($ONLINE[$loop_date][2])) {
      $online     = 1;
      $attendance = 1;
    }
...
function parse_scan_new($PIN, $date1, $date2, $SHIFT)
{
  $PIN        = db_escape($PIN);
  $date1      = db_escape($date1);
  $date2      = db_escape($date2);
  $LOG        = db_fetch(" SELECT DATE as unix_date, LOG_ID as id_log_mesin FROM log_mesin WHERE PIN='$PIN' AND (DATE(DATE) BETWEEN '$date1' AND '$date2') ");
  $EXCEPTION  = get_eksepsi($PIN, $date1, $date2);
  $LEMBUR     = get_lembur($PIN, $date1, $date2);
  $scan_array = array();
  foreach ($LOG as $key => $sc) {
  ...
  while ($time1 <= $time2) {
    ...
    $FINISH_BEGIN = isset($SHIFT[$PIN][$loop_date]->FINISH_BEGIN) ? $SHIFT[$PIN][$loop_date]->FINISH_BEGIN : '';
    $FINISH_END   = isset($SHIFT[$PIN][$loop_date]->FINISH_END) ? $SHIFT[$PIN][$loop_date]->FINISH_END : '';
    $LATITUDE     = isset($SHIFT[$PIN][$loop_date]->LATITUDE) ? $SHIFT[$PIN][$loop_date]->LATITUDE : '';
    $LONGITUDE    = isset($SHIFT[$PIN][$loop_date]->LONGITUDE) ? $SHIFT[$PIN][$loop_date]->LONGITUDE : '';
    foreach (array(
      'TO_IN',
      'TO_OUT',
      'TS',
    ...
    if (($check_in_date > $check_out_date) or $OVERNIGHT == 'YES') {
      $new_loop_date  = date('Y-m-d', strtotime($loop_date . ' +1 day'));
      $check_out_date = strtotime($new_loop_date . ' ' . $check_out);
      $NEW_LOG        = db_fetch(" SELECT DATE as unix_date, LOG_ID as id_log_mesin FROM log_mesin WHERE PIN='$PIN' AND DATE(DATE)= '$new_loop_date' ");
    ...
    if (valid_time($check_in) and valid_time($check_out)) {
      $working_day       = 1;
      $scan_in_candidate = $scan_out_candidate = $log_id_in_candidate = $log_id_out_candidate = array();
      if (count($scan_array) > 0) {
        foreach ($scan_array as $sa) {
          if ($sa['scan_time'] >= $scan_in_start_date and $sa['scan_time'] <= $scan_in_finish_date) {
            $scan_in_candidate[] = $sa['scan_time'];
...
function parse_scan_app($PIN, $date1, $date2, $SHIFT)
{
  $PIN        = db_escape($PIN);
  $date1      = db_escape($date1);
  $date2      = db_escape($date2);
...
function get_online_absent($PIN, $date1, $date2)
{
  $rs = db_fetch(" SELECT *
      FROM tabel_absen
      WHERE FROM_TBL = '1' AND STATUS = '1' AND APPROVE = '1' AND ID_KARYAWAN ='$PIN' AND (TGL_JADWAL >= '$date1' AND TGL_JADWAL <= '$date2')
    ");

  $ONLINE = array();
  if (count($rs) > 0) {
    foreach ($rs as $row) {
      $ONLINE[$row->TGL_JADWAL][$row->JENIS] = $row;
    }
  }
  return $ONLINE;
}
