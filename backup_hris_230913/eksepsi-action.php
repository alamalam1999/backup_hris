...
if ($OP == 'approve') {
	...
	*/
	...
	if (is_array($EKSEPSI_ID)) {
		$CU = current_user();
		$TIME = date('Y-m-d H:i:s');
		db_execute(" UPDATE eksepsi SET STATUS='APPROVED', UPDATED_BY='$CU->NAMA', UPDATED_ON='$TIME', APPROVED_BY='$CU->NAMA', APPROVED_ON='$TIME' WHERE EKSEPSI_ID IN (" . implode(',', $IDEKSEPSI_IDS) . ")");
	}
	header('location: eksepsi.php');
	exit;
}
