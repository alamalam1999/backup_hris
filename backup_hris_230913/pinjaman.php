	...
	<div class="row" style="margin:10px 0;">
		<div class="col-sm-2">
			<div class="dropdown">
				...
				<li><a href="pinjaman-action.php?op=add" style=""><i class="fa fa-plus"></i>&nbsp;&nbsp;Add</a></li>
				...
				<?php if(has_access('pinjaman.delete')){ ?>
		...
		<div class="col-sm-2">
			<?php echo dropdown('PROJECT_ID',project_option_filter(0),get_search('PINJAMAN','PROJECT_ID'),' id="PROJECT_ID" class="form-control input-sm" ') ?>
		</div>
		<div class="col-sm-2">
			<input type="text" id="NAMA" value="<?php echo get_search('PINJAMAN','NAMA') ?>" class="form-control input-sm" placeholder="Nama....">
		</div>
		<div class="col-sm-6">
			<h1 style="margin:0;text-align:right;">Pinjaman</h1>
		</div>
	</div>
...
$(document).ready(function(){
	$('#t').datagrid({
		queryParams: {'PROJECT_ID': $('#PROJECT_ID').val(), 'NAMA': $('#NAMA').val() },
		url:'pinjaman-json.php',
		fit: true,
		border: true,
		nowrap: false,
		striped: true,
		...
		columns:[[
			...
			{field:'JENIS_POTONGAN',title:'Jenis Potongan',width:200,sortable:true,align:'left'},
			{field:'GRAND_TOTAL',title:'Total',width:100,sortable:false,align:'right'},
			...
			{field:'PERIODE_MULAI',title:'Periode Mulai',width:120,sortable:false,align:'left'},
			{field:'PERIODE_SELESAI',title:'Periode Selesai',width:120,sortable:false,align:'left'},
	...
	$('#btn-edit').click(function(){
		...
		else window.location = 'pinjaman-action.php?op=edit&id='+sel.PINJAMAN_MASTER_ID;
		return false;
	});
	$('#btn-delete').click(function(){
		...
		{
			var REASON = prompt("Kenapa pinjaman ini di VOID ?", "Alasan");
			if (REASON != null) {
				window.location = 'pinjaman-action.php?op=delete&id='+sel.PINJAMAN_MASTER_ID+'&reason='+REASON;
			}
		}
		return false;
	});
	...
	$('#btn-approved').click(function(){
		...
		if( sel == undefined ) alert('No item selected');
			...
			$.each(rows, function(index, value){
				QS += '&ids[]=' + value.PINJAMAN_MASTER_ID;
			});
			window.location = 'pinjaman-action.php?op=approve' + QS;
		}
		return false;
	});
	/* end quick approved */ 
