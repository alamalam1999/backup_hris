...
if($OP=='edit'){
	...
	$EDIT = db_first(" SELECT * FROM pinjaman WHERE PINJAMAN_ID='$ID' ");
...
if($OP=='approve'){
	...
	if( is_array($IDS) )
	{
		...
		db_execute(" UPDATE pinjaman SET STATUS='APPROVED',UPDATED_BY='$CU->NAMA',UPDATED_ON='$TIME',APPROVED_BY='$CU->NAMA',APPROVED_ON='$TIME' WHERE PINJAMAN_ID IN (".implode(',',$IDS).")");
	}
...
if($OP=='delete'){
	...
	db_execute(" UPDATE pinjaman SET STATUS='VOID',KETERANGAN='Alasan Void : $REASON' WHERE PINJAMAN_ID='$ID' ");
	header('location: pinjaman.php');
	exit;
}
...
if( isset($_SERVER['REQUEST_METHOD']) AND strtoupper($_SERVER['REQUEST_METHOD'])=='POST' )
{
	...
	if( $ERROR_REQUIRE ){
		...
		if($OP=='' OR $OP=='add')
		{
			...
			/*db_execute(" INSERT INTO pinjaman (".implode(',',$FIELDS).") VALUES (".implode(',',$INSERT_VAL).") ");
			$OP = 'edit';
			$ID = $DB->Insert_Id();
		...
		/*else
		{
			db_execute(" UPDATE pinjaman SET ".implode(',',$UPDATE_VAL)." WHERE PINJAMAN_ID='$ID' ");
...
<section class="container-fluid" style="margin-top:70px;">
	<h1 class="border-title">
		...
		<?php if($OP=='edit'){ echo '<a href="pinjaman-action.php?op=add" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;Add New</a>'; } ?>
	</h1>
	<?php include 'msg.php' ?>
	...
	<div class="row">
		<div class="col-md-5">
			...
			<div class="form-group">
				...
				<div class="col-sm-8">
					<?php echo dropdown('PERIODE_ID',periode_option(),set_value('AKHIR_PERIODE_ID',$EDIT->AKHIR_PERIODE_ID),' id="AKHIR_PERIODE_ID" class="form-control" ') ?>
				</div>
			</div>
			<div class="form-group">
			...
			*/ ?>
			<div class="form-group">
				<label for="" class="col-sm-4 control-label">Pembayaran perbulan</label>
				<div class="col-sm-8">
		...
		<div class="col-md-5">
			...
			<?php if(has_access('pinjaman.change_status') && $OP == 'edit'){ ?>
			<div class="form-group">
