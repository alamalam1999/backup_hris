...
set_search('INTERVIEW', array('sort','order','NAMA','JK','POSISI_ID','LOWONGAN_ID','USIA'));
if( get_input('clear') ) clear_search('INTERVIEW', array('NAMA','JK','POSISI_ID','LOWONGAN_ID','USIA'));
...
$wh[] = " (KEPUTUSAN = 'TIDAK DISARANKAN' OR KEPUTUSAN = '') ";
...
$STATUS = array(
	''					=> '<span class="text-info"><b>PANGGILAN INTERVIEW</b></span>',
	'TIDAK DISARANKAN'	=> '<span class="text-warning"><b>TIDAK DISARANKAN</b></span>',
);

$rows = $t = array();
$key = 0;
if(count($rs)>0){ foreach($rs as $row){
	$t['STATUS_INTERVIEW'] = $STATUS[$row->STATUS_INTERVIEW];
	$t['TGL_INTERVIEW'] = tgl($row->TGL_INTERVIEW);
