	...
	<div class="row" style="margin:10px 0;">
		<div class="col-sm-2">
			<div class="dropdown">
				...
				<li><a href="kelebihan-jamajar-action.php?op=add" style=""><i class="fa fa-plus"></i>&nbsp;&nbsp;Add</a></li>
				<li><a href="javascript:void(0)" id="btn-edit" style=""><i class="fa fa-pencil"></i>&nbsp;&nbsp;Edit</a></li>
				<li role="separator" class="divider"></li>
				<li><a href="kelebihan-jamajar-import.php" id="btn-import"><i class="fa fa-file-excel-o"></i>&nbsp;&nbsp;Import from excel</a></li>
		...
		<div class="col-sm-2">
			<?php echo dropdown('PERIODE_ID',dropdown_option('periode','PERIODE_ID','PERIODE','ORDER BY PERIODE_ID DESC'),get_search('KELEBIHAN_JAMAJAR','PERIODE_ID'),' id="PERIODE_ID" class="form-control input-sm" ') ?>
		</div>
		<div class="col-sm-2">
			<?php echo dropdown('PROJECT_ID',project_option_filter(0),get_search('KELEBIHAN_JAMAJAR','PROJECT_ID'),' id="PROJECT_ID" class="form-control input-sm" ') ?>
		</div>
		<div class="col-sm-2">
			<input type="text" id="NAMA" value="<?php echo get_search('KELEBIHAN_JAMAJAR','NAMA') ?>" class="form-control input-sm" placeholder="Nama....">
		</div>
		<div class="col-sm-4">
			<h1 style="margin:0;text-align:right;">Kelebihan Jam Ajar</h1>
		</div>
	</div>
...
$(document).ready(function(){
	$('#t').datagrid({
		...
		url:'kelebihan-jamajar-json.php',
		fit: true,
		border: true,
		nowrap: false,
		striped: true,
		...
		columns:[[
			...
			{field:'RATE',title:'Rate',width:100,sortable:false,align:'right'},
			{field:'NILAI',title:'Nilai',width:100,sortable:false,align:'right'},
	...
	$('#btn-edit').click(function(){
		...
		else window.location = 'kelebihan-jamajar-action.php?op=edit&id='+sel.KEL_JAM_AJAR_ID;
		return false;
	});
	$('#btn-delete').click(function(){
		...
		{
			var REASON = prompt("Kenapa adjusment ini di VOID ?", "Alasan");
			if (REASON != null) {
				window.location = 'adjusment-action.php?op=delete&id='+sel.KEL_JAM_AJAR_ID+'&reason='+REASON;
			}
		}
		return false;
	});
	...
	$('#btn-approved').click(function(){
		...
		if( sel == undefined ) alert('No item selected');
			...
			$.each(rows, function(index, value){
				QS += '&ids[]=' + value.KEL_JAM_AJAR_ID;
			});
			window.location = 'kelebihan-jamajar-action.php?op=approve' + QS;
		}
		return false;
	});
