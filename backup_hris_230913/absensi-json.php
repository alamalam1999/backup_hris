	...
	foreach ($rs as $row) {
		...
		if (count($RANGE) > 0) {
			foreach ($RANGE as $date) {
				...
				$early = '';
				if ($_LATE > 0) $early = 'text-decoration:underline;';
				if ($_EARLY > 0) $early = 'text-decoration:underline;';
				$t['TGL_' . date('Ymd', strtotime($date))] = '<div style="color:#000000;background-color:' . $bg . ';' . $early . ';">' . $SCAN_IN . '-' . $SCAN_OUT . '</div>';

				if ($_ABS == '1') {
					$t['TGL_' . date('Ymd', strtotime($date))] = '<div style="background-color:red;color:#ffffff;text-align:center;">ABS</div>';
				}
				if ($_OFF == '1') {
