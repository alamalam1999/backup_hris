...
$MODULE = 'EQUIPMENT-USED';
is_login('equipment-used.view');
...
<section class="container-fluid" style="margin-top:70px;">
	<h1>
		<span style="color:#aaaaaa;">Equipment</span> - Equipment Used
		<a href="equipment-used-action.php?op=add" class="btn btn-primary" style="margin-left:10px;"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;Add New</a>
	</h1>

	<form id="form" action="equipment-used.php" method="GET">
	...
	<tr>
		...
		<td>
			<?php 
			$EQUIPMENT_USED = db_fetch("SELECT ED.*,E.NAMA FROM equipment_used_detail ED LEFT JOIN equipment E ON E.EQUIPMENT_ID=ED.EQUIPMENT_ID WHERE ED.EQUIPMENT_USED_ID='$row->EQUIPMENT_USED_ID'");
			foreach($EQUIPMENT_USED as $key){
				echo $key->NAMA.' ('.$key->QTY.')<br>';
			}
		...
		<td style="text-align:center;">
			<a href="equipment-used-action.php?op=edit&id=<?php echo $row->EQUIPMENT_USED_ID ?>" title="Update"><span class="label label-primary">UPDATE</span></a>
			<a href="equipment-used-action.php?op=delete&id=<?php echo $row->EQUIPMENT_USED_ID ?>" onclick="return confirm('Yakin data akan di hapus?')" title="Delete">
