...
if($OP=='edit'){
	$EDIT = db_first(" SELECT * FROM provinsi WHERE PROVINSI_ID='$ID' ");
}
if($OP=='delete'){
	db_execute(" DELETE FROM provinsi WHERE PROVINSI_ID='$ID' ");
...
if( isset($_SERVER['REQUEST_METHOD']) AND strtoupper($_SERVER['REQUEST_METHOD'])=='POST' )
{
	...
	$PROVINSI_ID = db_escape(get_input('PROVINSI_ID'));
	if( $ERROR_REQUIRE ){
		...
		if($OP=='' OR $OP=='add')
		{
			db_execute(" INSERT INTO provinsi (".implode(',',$FIELDS).") VALUES (".implode(',',$INSERT_VAL).") ");
			$OP = 'edit';
			$ID = $DB->Insert_Id();
		...
		{
			db_execute(" UPDATE provinsi SET ".implode(',',$UPDATE_VAL)." WHERE PROVINSI_ID='$ID' ");
...
<section class="container-fluid" style="margin-top:70px;">
	<h1 class="border-title">
		<?php echo ucfirst($OP) ?> Provinsi
		<a href="provinsi.php" class="btn btn-warning" style="margin-left:10px;">&laquo; Back</a>
		<button class="btn btn-primary" onclick="$('#form').submit()"><span class="glyphicon glyphicon-floppy-disk"></span>&nbsp;&nbsp;Save</button>
		<?php if($OP=='edit'){ echo '<a href="provinsi-action.php?op=add" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;Add New</a>'; } ?>
	</h1>
	<?php include 'msg.php' ?>
	<div style="border-top:1px dashed #cccccc; margin-top:15px;">&nbsp;</div>
	<form id="form" class="form-horizontal" action="provinsi-action.php?op=<?php echo $OP ?>&id=<?php echo $ID ?>" method="POST">
		<input type="hidden" name="CURR_ID" value="<?php echo $ID ?>">
		<div class="form-group">
			<label for="inputPassword" class="col-sm-2 control-label">Provinsi</label>
			<div class="col-sm-10">
