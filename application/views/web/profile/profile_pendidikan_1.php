    ...
    <div class="table-responsive">
      <table class="table table-bordered formal">
        <tr style="background-color: #E9ECEF; color: #495057;">
          <th colspan="6">Pendidikan formal</th>
        ...
        <tr>
          ...
          <th rowspan="2" class="text-center" style="width: 180px;">File</th>
          <th rowspan="2" class="text-center">Jurusan</th>
          <th rowspan="2" class="text-center">Institusi</th>
        ...
        <?php
          ...
          foreach ($PEND_FORMAL->result() as $key => $row) { 
            $bg_tr = '';
            if($row->APPROVED != 'APPROVED' && $row->KETERANGAN_APPROVED != '')$bg_tr = 'text-white bg-warning';
            ?>
            <tr class="<?= $bg_tr ?>">
              <td><?php echo $key + 1 ?></td>
              <td><?= $row->TINGKAT ?></td>
              <td>
                <?php if (!empty($row->FILE_PENDIDIKAN) and url_exists(hris('url') . 'uploads/karyawan/' . rawurlencode($row->FILE_PENDIDIKAN))) { ?>
                              <a class="btn btn-success btn-flat btn-sm" href="<?php echo hris() . "uploads/karyawan/" . $row->FILE_PENDIDIKAN ?>" title="Download" download>
                             Download 
                            </a>
                          <?php } ?>
...
<div class="modal fade" id="modal_1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form class="" action="<?= base_url('profile_pendidikan/proses_add_pend_formal') ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <table class="table ">
                    ...
                    <tr>
                      <td >File</td><td>:</td>
                      <td>
                        <input type="file" name="FILE_PENDIDIKAN"  class="form-control">
